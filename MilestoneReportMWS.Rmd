...
```
##Generate table showing the basic statistics 
...
Next, we will obtain the same set of statistics for a sample of the data.
I could sample by number of characters or words, but I will sample by line count.
...
## e.g "â", "o", "î", "z","???","T","³","ð","¾","ñ", "~"...
## when I used the tolower function.
## I tried the stringi package stri_trans_tolower function...
## Not sure which one is better, I think the problem may be...
## in the data before I began preprocessing.

## Convert everything to lowercase 
#dat10Lower<-sapply(dat10NoStop, tolower)
#dat10Lower<- tm_map(corp, tolower)
#dat10Lower<- tolower(dat10NoStop)
#corp<-Corpus(VectorSource(dat10NoStop))
#print(dat10NoStop)
library(stringi)
...
dat10NoShort <- removeWords(dat10NoWS2, "\\b\\w{1,2}\\b") 
...
barplot(dfUniSort[1:20,2],col="lightblue",
...
p <- plot_ly(x = dfUniSort[1:20,1], type = "histogram")

p
remove(freqCount,freqNames)
p <- plot_ly(alpha=0.6) %>%
  add_histogram(x=dfUni$Word) %>%
  add_histogram(dfBi$Word) %>%
  add_histogram(dfTri$Word) %>%
  layout(barmode = "overlay")

## Word clouds are largely useless in my opinion, but why not show a few? :)
